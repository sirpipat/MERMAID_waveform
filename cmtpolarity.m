function [fp,fsv,fsh,v] = cmtpolarity(M, evla, evlo, evdp, stla, stlo, model)
% [fp,fsv,fsh,v] = CMTPOLARITY(M, evla, evlo, evdp, stla, stlo, model)
%
...
% is_down       whether the take-off path heading downward or not
% phase         phase name of the first arrival at the station
function [azim, dist, theta, is_down, phase] = ...
    expensivefunction(evla, evlo, evdp, stla, stlo, model)
% CONSTANTS
R_EARTH = 6371; % km
...
for ii = 1:numel(stlo)
    ...
    tt = indeks(tauptime('mod', model, 'depth', evdp, ...
        ...
        'sta', [stla(ii) stlo(ii)]), 1);
    rayparameter = tt.rayparameter * 180 / pi;
    is_down(ii) = strcmp(tt.phase(1), 'P');
    phase{ii} = tt.phase;
    if strcmpi(model, 'ak135')
        vp = ak135('depths', evdp, 'dcbelow', is_down(ii)).vp;
    elseif strcmpi(model, 'iasp91')
        vp = iasp91('depths', evdp, 'dcbelow', is_down(ii)).vp;
    elseif strcmpi(model, 'prem')
        vp = prem('depths', evdp, 'dcbelow', is_down(ii)).vp;
    else
