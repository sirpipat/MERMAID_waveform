function ax = plotsac2(SeisData, HdrData, ax, varargin)
% ax = PLOTSAC2(SeisData, HdrData, ax, varargin)
%
...
% ARRIVAL2SAC, READSAC
%
% Last modified by Sirawich Pipatprathanporn, 10/29/2021
...
% plot sesimogram
ax = signalplot(x, fs, 0, ax, 'title', [], 'k', varargin{:});
...
while phaseTime ~= -12345
    vline(ax, seconds(dt_ref - dt_B) + phaseTime, 'LineStyle', '--', ...
        'LineWidth', 1, 'Color', 'r');
    
    [x_pos, y_pos] = norm2trueposition(ax, 1/100, 9/10);
    text(x_pos + seconds(dt_ref - dt_B) + phaseTime, y_pos, phaseName);
    phaseNum = phaseNum + 1;
    switch phaseNum
        case 1
