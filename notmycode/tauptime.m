...
%           tt=tauptime(...,'stdp|stadepth',depth,...)
%
%    Description:
...
%                      .rayparameter - ray parameter (sec/rad)
...
%
%     TT=TAUPTIME(...,'STDP|STADEPTH',DEPTH,...) sets the station depth
%     to DEPTH.  DEPTH should be in kilometers!  The default is 0.
%
%    Notes:
...
%         'sta',[-40 -100],'evt',[30,50], 'evdp', 5)
%
%    See also: TAUPPATH, TAUPCURVE, TAUPPIERCE, TAUPCREATE, TAUP

%     Version History:
%        Sep. 13, 2023 - added an option for station depth. Use TauP-2.3.0
%                        or newer
%                      - changed ray parameter output to s/rad
...
%     Last Updated Jan. 13, 2014 at 17:15 GMT
%
%     Last modified by sirawich-at-princeton.edu, 09/13/2023

% todo: implement STATION DEPTH in TAUPPATH, TAUPCURVE, TAUPPIERCE,
% TAUPCREATE, TAUP
    ...
    switch lower(varargin{i})
        ...
        case {'stdp' 'stadepth'}
            if(~isscalar(varargin{i+1}) ...
                    || ~isreal(varargin{i+1}) || varargin{i+1}<0)
                error('TauP:tauptime:badInput',...
                    'STADEPTH must be a positive number (in km)!');
            end
            stdp=varargin{i+1};
        otherwise
...
tt(1:narr,1)=struct('modelname',[],'event',[],'station',[],'depth',[],...
    'stadepth',[],'distance',[],'mindistance',[],'phase',[],...
    'puristphase',[],'time',[],'rayparameter',[]);
for ii=1:narr
    % get phase info
    ...
    tt(ii).rayparameter=arr.getRayParam;
end
