...
if plt
    ...
    hold on
    grid on
    p2 = plot(t_relative + t_shift1(1), pres_s1 * s1(1), 'LineWidth', 0.5, ...
        'Color', red);
    p3 = plot(t_relative + t_shift2(1), pres_s2 * s2(1), 'LineWidth', 1, ...
        'Color', blue);
    xlim([-10 25])
    wh = and(t_relative >= window_waveform(1), ...
        t_relative <= window_waveform(2));
    wh1 = and(t_relative + t_shift1(1) >= window_waveform(1), ...
        t_relative + t_shift1(1) <= window_waveform(2));
    wh2 = and(t_relative + t_shift2(1) >= window_waveform(1), ...
        t_relative + t_shift2(1) <= window_waveform(2));
    ylim([-1.2 1.2] * max([max(abs(pres_o(wh))), ...
                           max(abs(s1(1) * pres_s1(wh1))), ...
                           max(abs(s2(1) * pres_s2(wh2))) ...
                          ]));
    
    label2 = sprintf('$$ \\textnormal{%s} : \\tau^W = %.2f~\\textnormal{s, X}^W = %.2f $$', la{1}, t_shift1(1), CCmax1(1));
    label3 = sprintf('$$ \\textnormal{%s} : \\tau^W = %.2f~\\textnormal{s, X}^W = %.2f $$', la{2}, t_shift2(1), CCmax2(1));
    ...
    vline(ax5, t_shift1(1), 'LineStyle', ':', ...
        'Color', [0.4 0.4 0.4], 'LineWidth', 1.);
    hline(ax5, CCmax1(1), 'LineStyle', ':', ...
        'Color', [0.4 0.4 0.4], 'LineWidth', 1.);
    
    vline(ax6, t_shift2(1), 'LineStyle', ':', ...
        'Color', [0.4 0.4 0.4], 'LineWidth', 1.);
    hline(ax6, CCmax2(1), 'LineStyle', ':', ...
        'Color', [0.4 0.4 0.4], 'LineWidth', 1.);
